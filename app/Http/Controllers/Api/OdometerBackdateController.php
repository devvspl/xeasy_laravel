...
class OdometerBackdateController extends Controller
{
    public function odoBackdateSubmission($yearId, $employeeId)
    {
        ...
        $records = DB::table("{$tableName} as e")->join('hrims.hrm_employee as emp', 'emp.EmployeeID', '=', 'e.CrBy')->join('hrims.hrm_employee_general as gen', 'gen.EmployeeID', '=', 'e.CrBy')->leftJoin('hrims.core_departments as dep', 'gen.DepartmentId', '=', 'dep.id')->join("{$monthExpenseTable} as mef", function ($join) {
            $join->on('mef.EmployeeID', '=', 'e.CrBy')->on('mef.Month', '=', 'e.ClaimMonth')->where('mef.Status', '=', 'Open');
        })->select('e.CrBy', 'e.ExpId', 'e.ClaimYearId', 'e.BillDate', 'e.CrDate', 'e.opening_filepath', 'e.closing_filepath', 'e.odomtr_opening', 'e.odomtr_closing', 'e.TotKm', 'e.RatePerKM', 'e.ClaimStatus', 'e.Backdate_Odometer_Status', DB::raw('DATEDIFF(e.CrDate, e.BillDate) AS days_difference'), DB::raw("CONCAT(emp.Fname, ' ', emp.Sname, ' ', emp.Lname) AS employee_name"), 'emp.EmpCode', 'mef.Status as monthexpense_status', 'mef.Month as monthexpense_month')->where('e.ClaimId', 7)->whereColumn('e.BillDate', '!=', 'e.CrDate')->whereDate('e.BillDate', '>=', $effectiveDate)->whereIn('e.CrBy', $employeeIds)->whereNotNull('e.BillDate')->whereNotIn('e.ClaimStatus', ['Deactivate'])->get();
        $grouped = $records->groupBy('CrBy')->map(function ($items) use ($approvalType) {
            $firstItem = $items->first();

            $statusCounts = [
                'Pending' => $items->where('Backdate_Odometer_Status', 'P')->count(),
                'Approved' => $items->where('Backdate_Odometer_Status', 'A')->count(),
                'Rejected' => $items->where('Backdate_Odometer_Status', 'R')->count(),
            ];
            ...
            return [
                ...
                'items' => $items->map(function ($item) {
                    ...
                    return [
                        ...
                        'CrDate' => $item->CrDate,
                        'odomtr_opening' => $item->odomtr_opening,
                        'odomtr_closing' => $item->odomtr_closing,
                        'opening_filepath' => Storage::disk('s3')->url("Expense/{$item->ClaimYearId}/{$item->CrBy}/{$item->opening_filepath}"),
                        'closing_filepath' => Storage::disk('s3')->url("Expense/{$item->ClaimYearId}/{$item->CrBy}/{$item->closing_filepath}"),
                        'TotKm' => $item->TotKm,
                        'RatePerKM' => $item->RatePerKM,
                        'ClaimStatus' => $item->ClaimStatus,
                        'days_difference' => $item->days_difference,
                        'Backdate_Odometer_Status' => $status,
                    ];
                })->toArray(),
            ];
        });
    ...
    public function odoBackdateBulkApproval($yearId, $employeeId)
    {
        ...
        $updatedCount = $model->where('ClaimId', 7)
            ->where('Backdate_Odometer_Status', 'P')
            ->update([
                'Backdate_Odometer_Status' => 'A',
                'Backdate_Odometer_By' => $employeeId,
                'Backdate_Odometer_Date' => date('Y-m-d'),
            ]);

        return $this->jsonSuccess(
            ['updated_count' => $updatedCount],
            'Bulk odometer backdate approval completed successfully.'
        );
    }
    ...
    public function odoBackdateBulkRejection($yearId, $employeeId)
    {
        ...
        return $this->jsonSuccess(
            ['updated_count' => $updatedCount],
            'Bulk odometer backdate rejection completed successfully.'
        );
    }
