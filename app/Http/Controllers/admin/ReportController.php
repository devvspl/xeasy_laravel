        ...
        private function buildClaimQuery(array $filters, string $table, bool $forCount = false, $columns = null) {
            ...
            if ($forCount) {
                ...
                if (in_array('emp_name', $columns)) {
                    $selects[] = DB::raw("MAX(CONCAT(hrims.hrm_employee.Fname, ' ', COALESCE(hrims.hrm_employee.Sname, ''), ' ', hrims.hrm_employee.Lname)) as employee_name");
                }
    ...
    public function export(Request $request) {
        ...
        try {
            ...
            $export = match($reportType) {
                'month_wise' => new MonthWiseClaimReportExport($query, $filters, $columns, $protectSheets, $table), 'department_wise' => new DepartmentWiseClaimReportExport($query, $filters, $columns, $protectSheets, $table), 'claim_type_wise' => new ClaimTypeWiseClaimReportExport($query, $filters, $columns, $protectSheets, $table), 'head_wise' => new HeadWiseClaimReport($query, $filters, $columns, 'Claims', $protectSheets, $table),
            default => new ClaimReportExport($query, $filters, $columns, 'Claims', $protectSheets, $table),
        };
        $reportName = match($reportType) {
            'month_wise' => 'MonthWise_Claims', 'department_wise' => 'DepartmentWise_Claims', 'claim_type_wise' => 'ClaimTypeWise_Claims', 'head_wise' => 'Headwise_Claim',
