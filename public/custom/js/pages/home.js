    ...
    function fetchDashboardData(billDateFrom, billDateTo) {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (departmentTotals.length) {
                    ...
                    const totalY7 = departmentTotals.reduce(
                        (sum, dept) =>
                            sum +
                            parseFloat(
                                dept["TotalFinancedTAmt_Y" + yearId] || 0
                            ),
                        0
                    );
                ...
                if (
                    departmentChart &&
                    typeof departmentChart.destroy === "function"
                ) {
                    departmentChart.destroy();
                }
                if (
                    claimTypeChart &&
                    typeof claimTypeChart.destroy === "function"
                ) {
                    ...
                    series: [
                        ...
                        },
                        {
                            ...
                            data: departmentTotals.map(
                                (item) =>
                                    item[
                                        "TotalFinancedTAmt_Y" + previousYearId
                                    ] || 0
                            ),
                        },
                        {
                ...
                var claimTypeOptions = {
                    ...
                    xaxis: {
                        categories: claimTypeTotals.map(
                            (item) => item.ClaimCode
                        ),
                        axisTicks: {
                            show: false,
